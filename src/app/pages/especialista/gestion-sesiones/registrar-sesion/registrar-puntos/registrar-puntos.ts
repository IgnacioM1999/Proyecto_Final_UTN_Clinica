...
import { SindromesServices } from '../../../../../services/sindromes';
import { AntecedentesServices } from '../../../../../services/antecedentes';
...
export class RegistrarPuntos implements OnInit {
  ...
  constructor(private pasantesServices: PasantesServices, private turnosServices: TurnosServices, private insumosServices: InsumosServices, private sesionesServices: SesionesServices,
    private sindromesServices: SindromesServices, private tratamientosServices: TratamientosServices, private antecedentesServices: AntecedentesServices, private router: Router) { }
  ...
  registrar() {
    ...
    // Crear síndrome si no existe
    const obtenerIdSindrome = () => {
      if (sindrome.idSindrome) {
        return Promise.resolve(sindrome.idSindrome);
      } else {
        return new Promise<number>((resolve, reject) => {
          this.sindromesServices.createSindrome({
            idSindrome: 0,
            descripcion: sindrome.descripcion
          }).subscribe({
            next: (resp: any) => resolve(resp.idSindrome),
            error: reject
          });
        });
      }
    ...
    // Crear tratamiento si no existe
    const obtenerIdTratamiento = () => {
      if (tratamiento.idTratamiento) {
        return Promise.resolve(tratamiento.idTratamiento);
      } else {
        return new Promise<number>((resolve, reject) => {
          this.tratamientosServices.createTratamiento({
            idTratamiento: 0,
            nombre: tratamiento.nombre,
            descripcion: tratamiento.descripcion,
            puntos: tratamiento.puntos
          }).subscribe({
            next: (resp: any) => resolve(resp.idTratamiento),
            error: reject
          });
        });
      }
    ...
    // Esperar a que ambos IDs estén disponibles
    Promise.all([obtenerIdSindrome(), obtenerIdTratamiento()])
      .then(([nuevoIdSindrome, nuevoIdTratamiento]) => {
        ...
        this.sesionesServices.createSesion(nuevaSesion).subscribe({
          next: (response) => {
            ...
            const antecedentes = cuestionario.antecedentes || {
              personales: '',
              familiares: '',
              quirurgicos: ''
            };

            const dniPaciente = seleccionTurno.dniPaciente;
            const listaAntecedentes: any[] = [];

            if (antecedentes.personales.trim() !== '') {
              listaAntecedentes.push({
                tipoAntecedente: 'Personales',
                descripcion: antecedentes.personales,
                dniPaciente
              });
            }
            if (antecedentes.familiares.trim() !== '') {
              listaAntecedentes.push({
                tipoAntecedente: 'Familiares',
                descripcion: antecedentes.familiares,
                dniPaciente
              });
            }
            if (antecedentes.quirurgicos.trim() !== '') {
              listaAntecedentes.push({
                tipoAntecedente: 'Quirúrgicos',
                descripcion: antecedentes.quirurgicos,
                dniPaciente
              });
            }

            let registroAntecedentes: Observable<any>;

            if (listaAntecedentes.length > 0) {
              registroAntecedentes = forkJoin(
                listaAntecedentes.map(a =>
                  this.antecedentesServices.createAntecedente(a)
                )
              );
            } else {
              registroAntecedentes = of(null); // no hay antecedentes
            }

            registroAntecedentes.subscribe({
              next: () => {
                console.log("Antecedentes registrados correctamente.");
                ...
                Swal.fire({
                  icon: 'warning',
                  title: 'Atención',
                  text: 'La sesión se registró pero ocurrió un error al registrar antecedentes.',
                  confirmButtonColor: '#ffc107'
                });
              }
            });
